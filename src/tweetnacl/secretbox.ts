...
export function secretbox(msg: ByteArray, nonce: ByteArray, key: ByteArray): ByteArray {
  ...
  const m = new ByteArray(SecretBoxLength.Zero + msg.length);
  const c = new ByteArray(m.length);
...
export function secretbox_open(
  ...
  const c = new ByteArray(SecretBoxLength.Overhead + box.length);
  const m = new ByteArray(c.length);
